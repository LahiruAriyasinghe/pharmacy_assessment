...
$('#start_at').change(function (e) { 
    let start = $(this).val();
    let end = $('#end_at').val();    
    let times = start.split(":");

    let hour = (parseInt(times[0]) != 24) ? parseInt(times[0])+1 : 0;
    hour = hour < 10 ? '0'+hour : hour;

    let endTime = `${hour}:${times[1]}`
        ...
        success: function (response) {
            ...
            if ("success" in response) {
                ...
                Swal.fire(
                    ...
                    if (result.value) {
                        table.ajax.reload();
                        $('#new_session_model').modal('toggle');
...
function initForm() {
    ...
    patientMask.destroy();
    patientMask = IMask(document.getElementById('maximum_patients'), {
      mask: Number,
      min: 0,
      max: 1000,
      thousandsSeparator: ''
