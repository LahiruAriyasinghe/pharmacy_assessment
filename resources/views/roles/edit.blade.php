...
<div class="container">
    <div class="row">
        <div class="col-12">
            <form action="{{route('roles.update', $role)}}" method="post" id="role_form">
                @csrf
                @method('PUT')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-4">
                        <input id="role_name" type="text" class="form-control" name="role_name" value="{{substr($role->name, strpos($role->name, "-") + 1)}}" required>
                    </div>
                </div>
                <hr>
                <div class="row my-4">
                        ...
                        <div class="permisions-role-card shadow-sm">
                            <div class="permisions-role-card-top">
                                ...
                                <div class="custom-control custom-checkbox">
                                    <input class="custom-control-input permission-category" type="checkbox" value="{{$key}}" 
                                    id="permission-{{Illuminate\Support\Str::kebab($key)}}">
                                ...
                                <div class="row pl-3">
                                    ...
                                    <div class="col-lg-6 col-md-12 custom-control custom-checkbox">
                                        <input type="checkbox" class="permission-{{Illuminate\Support\Str::kebab($key)}} custom-control-input permission" id="role_permissions_{{$permission->id}}"
                                            name="role_permissions[]" value="{{$permission->id}}" @if($role->hasPermissionTo($permission->id)) checked @endif>
                ...
                <button type="button" id="role_save_btn" class="btn btn-primary">Update Role</button>
                </div>
            </form>
