...
@section('title', 'Manage Test Data Categories')

@section('content')
<div class="container">
    <div class="page-title-btn">
        <div class="page-title-with-button"><span><img class="page-title-icon"
                    src="{{ asset('img/lab-test-category.svg') }}"></span>Lab Test Data Categories</div>
        @can('create test data category', App\User::class)
        <Button type="button" class="btn btn-primary" data-toggle="modal" data-target="#new_category_model">Add
            New Category</Button>
        @endcan
    </div>
    <hr>
...
<div class="container">
    ...
    <div class="row">
        <div class="col-12">
            <table id="categories_table" class="table table-bordered hover" style="width:100%">
                <thead>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="">
                    <form action="{{route('test-data.categories.store')}}" method="post" id="categories_form">
                        @csrf
...
<form id="delete_category_form" action="{{route('test-data.categories.destroy', ['category'=>'xx'])}}" method="post">
    @csrf
    @method('DELETE')
</form>
...
<script src="{{asset('js/test-data/categories/create.js')}}"></script>
<script>
var table = null;
...
$(document).ready(function() {
    ...
    table = $('#categories_table').DataTable({
        "ajax": "{{route('resources.test-data.categories.index')}}",
        "columns": [{
                "data": "name"
            },
            {
...
function editCategory(categoryId) {
    let url = "{{route('test-data.categories.edit', ['category' => 'xx'])}}";
